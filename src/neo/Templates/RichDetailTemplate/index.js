...
class RichDetailTemplate extends Component {
  ...
  mergeDetail(detailList, respData) {
  ...
  getComment(detailData, respData) {
    const AllComment = detailData.map((item) => {
      ...
      if (respData[item.key] && respData[item.key] !== '') {
        mergedDetailList = this.mergeDetail(item.data, respData[item.key]);
      } else {
        mergedDetailList = this.mergeDetail(item.data, respData);
      }
      let comment = '';
...
export default RichDetailTemplate
